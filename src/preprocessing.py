...
# Ggestire percorsi, file e directory
import os

# Leggere i file .dat del dataset DEAP, che sono stati salvati utilizzando il formato pickle
import pickle

# Gestire gli array contenenti i segnali EEG e per effettuare operazioni numeriche
import numpy as np

# Importa la funzione che applica il filtraggio ai segnali EEG
# La funzione filter_data() viene definita nel modulo src/filtering.py e applica i filtri previsti dalla pipeline
...
# Il dataset contiene 32 canali EEG, mentre gli altri canali presenti nei dati 
# originali appartengono ad altre tipologie di segnali fisiologici
N_EEG_CHANNELS = 32

# Crea la directory dei dati preprocessati se non esiste
# exist_ok=True evita che venga generato un errore nel caso in cui la directory sia già presente
os.makedirs(
    PRE_DIR,
    exist_ok=True
)
...
def load_deap_subject(path):
    ...
    Parametri:
        path: percorso del file .dat del soggetto.
        
    Restituisce:
        data: segnali EEG e altri segnali fisiologici presenti nel file.
        labels: etichette associate ai 40 trial del soggetto.
    """

    # Apre il file in modalità binaria
    # I file DEAP sono memorizzati in formato pickle, quindi devono essere aperti con modalità "rb"
    with open(path, "rb") as f:

        # Carica il contenuto del file pickle
        # encoding="latin1" permette di leggere correttamente file pickle creati originariamente con Python 2
        d = pickle.load(
            f,
            encoding="latin1"
        )

    # Il dizionario DEAP contiene la chiave "data", che contiene i segnali registrati
    # Restituisce inoltre la chiave "labels", contenente le valutazioni associate ai trial
...
def load_deap_dataset(raw_dir=RAW_DIR, eeg_only=True):
    ...
    Parametri:
        raw_dir: directory contenente i file .dat.
        eeg_only: se True vengono mantenuti solamente i 32 canali EEG.

    Restituisce:
        X: array contenente i dati di tutti i soggetti.
        y: array contenente le label dei trial.
        subject_ids: identificativo del soggetto associato a ogni trial.
    """

    # Liste temporanee utilizzate per raccogliereprogressivamente i dati dei vari soggetti
    X_list, y_list, subj_list = [], [], []

    # Controlla che la directory specificata esista
    # Se la cartella non viene trovata, viene sollevato un errore FileNotFoundError
    if not os.path.isdir(raw_dir):
        raise FileNotFoundError(
            f"Cartella del dataset non trovata: {raw_dir}"
        )

    # Recupera tutti i file con estensione .dat presenti nella directory del dataset
    # sorted() garantisce che i file vengano elaborati in ordine alfabetico
    files = sorted(
    ...
    for filename in files:
        ...
        # Se eeg_only è True, mantiene solamente i primi 32 canali, corrispondenti ai canali EEG
        if eeg_only:
        ...
        # data.shape[0] rappresenta il numero di trial appartenenti al soggetto corrente
        # In questo modo ogni trial viene associato al soggetto corretto
        subj_list.extend(
            [subject_id] * data.shape[0]
        )

    # Unisce i dati di tutti i soggetti lungo la dimensione dei trial
    X = np.concatenate(
        X_list,
        axis=0
    )
    ...
    # Converte la lista degli identificativi dei soggetti in un array NumPy
    subject_ids = np.array(
        subj_list
    )

    # Restituisce i tre elementi principali utilizzati dalla pipeline:
    # - segnali
    # - label
...
def segment_signal(
    ...
    Parametri:
        data: array con forma (numero_trial, numero_canali, numero_campioni).
        segment_length: durata di ogni segmento in secondi.
        fs: frequenza di campionamento del segnale.
        overlap: percentuale di sovrapposizione tra segmenti. 0.0 significa nessuna sovrapposizione.

    Restituisce:
        segments: array contenente tutti i segmenti generati.
        trial_index: indice del trial originale a cui appartiene ogni segmento.
    """
    ...
    # Calcola lo spostamento temporale tra l'inizio di un segmento e quello successivo.
    # Se overlap = 0:
    # step = win
    #
    # Se overlap = 0.5:
    ...
    # Se lo step fosse 0 o negativo, il ciclo di segmentazione non potrebbe procedere correttamente
    if step <= 0:
        raise ValueError(
    ...
    # n_ch: numero di canali
    # n_samples: numero di campioni per trial
    n_trial, n_ch, n_samples = data.shape

    # Lista che conterrà tutti i segmenti generati
    segments = []

    # Lista che permetterà di mantenere il collegamento tra ogni segmento e il trial originale
    trial_index = []
    ...
    for t in range(n_trial):
        ...
        # Continua a creare segmenti finché rimane una finestra completa all'interno del trial
        while start + win <= n_samples:
            ...
            # Questo sarà necessario successivamente per associare le label corrette ai segmenti
...
def normalize_signal(data):
    ...
    La normalizzazione viene effettuata separatamente per ogni segmento/canale.

    La trasformazione utilizzata è: (x - media) / deviazione_standard
    """

    # Calcola la media lungo l'ultima dimensione, cioè lungo il tempo
    # keepdims=True mantiene la dimensione necessaria per poter effettuare
    # successivamente la sottrazione tramite broadcasting
    mean = data.mean(
        axis=-1,
        keepdims=True
    )
    ...
    # Evita una divisione per zero nei casi in cui un segnale abbia deviazione standard uguale a zero
    # In questi casi viene utilizzato un valore molto piccolo
    std[std == 0] = 1e-8

    # Applica la normalizzazione z-score:
    # valore normalizzato = (valore originale - media) / deviazione standard
    return (
        data - mean
    ) / std
...
def full_preprocess(
    ...
    I due passaggi possono essere attivati o disattivati tramite i parametri della funzione.
    """

    # Se apply_filter è True, applica il filtraggio notch + bandpass definito in filtering.py
    if apply_filter:
        data = filter_data(data)

    # Se apply_norm è True, applica la normalizzazione tramite z-score
    if apply_norm:
...
def preprocess_and_save_subject(
    ...
    Parametri:
        path: percorso del file .dat originale.
        out_dir: directory nella quale salvare il file preprocessato.
        eeg_only: se True vengono mantenuti solamente i 32 canali EEG.
...
# Questa condizione fa sì che il codice sottostante venga eseguito solamente 
# quando preprocessing.py viene avviato direttamente
# Se il modulo viene importato da un altro file, questa parte non viene eseguita
if __name__ == "__main__":
    ...
    # Messaggio finale che indica il completamento dell'elaborazione
    print(
