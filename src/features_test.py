...
# X: contiene i segnali EEG
# y: contiene le etichette associate ai trial, tra cui valence e arousal
# subject_ids: contiene l'identificativo del soggetto associato a ciascun trial
...
# fs=128 indica la frequenza di campionamento utilizzata per il segnale
# overlap=0.0 significa che i segmenti non si sovrappongono
# X_segments: contiene i segmenti EEG generati
# trial_idx: permette di sapere a quale trial originale appartiene ciascun segmento
...
# Questo permette di verificare manualmente a quali trial appartengono i primi segmenti generati
print(
    "Trial_idx primi 20:",
    trial_idx[:20]
)
...
# ============================================================
# 3. TEST DIAGNOSTICO: ESTRAZIONE DELLE FEATURE DI UN SINGOLO SEGMENTO
# ============================================================
...
# Analizza i primi 300 segmenti, oppure tutti i segmento se il dataset contiene meno di 300 segmenti
# range(0, min(300, len(X_segments))) permette quindi di limitare il test per evitare 
# di elaborare inutilmente l'intero dataset
for i in range(
    0,
    min(300, len(X_segments))
):

    # Ogni 50 segmenti viene stampato un messaggio per monitorare l'avanzamento dell'elaborazione
    if i % 50 == 0:
        print(
...
# Messaggio che conferma il completamento del test di estrazione delle feature per singolo segmento
print(
...
# y_arousal_seg: etichette di arousal associate ai segmenti
# subj_seg: identificativo del soggetto associato a ciascun segmento
...
# Verifica quanti valori NaN sono presenti nella matrice delle feature
# np.isnan() crea una matrice booleana: True dove trova un NaN e False negli altri casi
...
# Seleziona il primo segmento per effettuare un controllo manuale della corrispondenza tra:
# - trial originale
# - etichetta originale
...
# Stampa l'identificativo del trial originale al quale appartiene il segmento 0
print(
    "  trial_idx:",
    trial_idx[i]
)

# Recupera dalla matrice y la valence originale del trial associato al segmento
# trial_idx[i] indica quale trial originale corrisponde al segmento i
...
# Recupera dalla matrice y l'arousal originale del trial associato al segmento
...
# la corretta propagazione delle etichette durante la segmentazione
print(
    "  arousal segmentata:",
    y_arousal_seg[i]
)
...
# np.unique() restituisce tutti gli identificativi distinti dei soggetti presenti nei segmenti
# Questo controllo permette di verificare che gli ID dei soggetti siano stati 
# correttamente propagati dal dataset originale ai segmenti creati durante il preprocessing
print(
